...
export const strings = {
  ...
  files: {
    ...
    save: 'Save',
    saving: 'Saving…',
    /** The tree's own changes and the folder search (v4). */
    newFile: 'New file',
    newFolder: 'New folder',
    rename: 'Rename',
    delete: 'Delete',
    confirmDelete: (name: string): string => `Delete ${name}? Click again · Rewind can restore it`,
    namePlaceholder: 'name',
    badName: 'Use a plain name: no slashes and none of : * ? " < > |',
    exists: (name: string): string => `${name} already exists here`,
    createFailed: 'Could not create it',
    renamed: (from: string, to: string): string => `Renamed ${from} to ${to}`,
  ...
  rightPanel: {
    ...
    verify: {
      ...
      failed: 'Verify did not start',
      noFolder: 'Verify runs in a folder. Open one for this chat first.',
      empty: 'Nothing verified yet. Run verify to check the folder with its own build and tests, then have another AI review the change.',
      checksTitle: 'Checks',
      reviewTitle: 'Review by another AI',
      reviewer: 'Reviewer',
      noReviewer: 'No reviewer: connect a second provider to get an AI review',
      noChecks: 'No checks found in this folder.',
      status: { pending: 'waiting', running: 'running', pass: 'passed', fail: 'failed' } as const,
      reviewStatus: {
      ...
      verdict: {
        pass: 'No issues',
        issues: (count: number): string => (count === 1 ? '1 issue' : `${count} issues`),
        unreadable: 'Unreadable answer',
      },
      severity: { high: 'high', medium: 'medium', low: 'low' } as const,
      fix: 'Fix with a prompt',
      /** The prompt "Fix with a prompt" writes for an issue. */
      fixPrompt: (issue: { file: string; line: number | null; message: string; fix: string }): string =>
        `A review of the last change found this issue in ${issue.file}${issue.line === null ? '' : `:${issue.line}`}: ${issue.message}${issue.fix === '' ? '' : ` Suggested fix: ${issue.fix}`} Please fix it and verify the fix.`,
      passed: 'Passed',
      notPassed: 'Needs work',
      ms: (ms: number): string => (ms < 1000 ? `${ms}ms` : `${(ms / 1000).toFixed(1)}s`),
      footerChip: 'Verify with…',
      footerResult: (pass: boolean, engine: string, issues: number): string =>
